"""
all of this file except LBFGS class is from https://github.com/hjmshi/PyTorch-LBFGS/blob/master/functions/LBFGS.py
"""
...
class LBFGS(EPIOptimize):
    """
    EPI-MRI Distortion Correction using l-BFGS.

    Inherits from EPIOptimize to implement optimization functionality for EPI-MRI Distortion Correction
    ...
    def run_correction(self, B0):
        ...
        Bc_old = torch.clone(self.B0)

        tolG = 1e-4  # for stopping, gradient norm
        tolJ = 1e-6
        tolY = 1e-6

        lbfgs = FullBatchLBFGS([self.Bc], history_size=30)

        def closure():
            lbfgs.zero_grad()
            objective, grad = self.corr_obj.eval(self.Bc, do_derivative=True)
            self.Bc.grad = grad
            return objective

        loss_init = self.corr_obj.eval(self.B0, do_derivative=False).detach().item()
        self.log.log_iteration({'iteration': "Iteration", 'loss': "Loss Value", 'func evals': "Num Func Evals", 'LS success': "LS Success",
                                    'dist val': "Dist Val", 'reg val': "Reg Val", 'grad norm': "Grad Norm",
                                    'step size': "Step Size"})
        self.log.log_iteration({'iteration': 0, 'loss': loss_init})
        Jstop = abs(loss_init)

        fevals = 0
        for i in range(1, self.max_iter + 1):
            options = {'closure': closure, 'current_loss': torch.tensor(loss_init, dtype=self.Bc.dtype, device=self.Bc.device), 'max_ls': 25}
            F_new, g_new, t, ls_step, closure_eval, grad_eval, desc_dir, fail = lbfgs.step(options)
            stop_fmap_change = i > 1 and (torch.norm(self.Bc - Bc_old)) <= tolY * (1 + torch.norm(B0))
            self.log.log_iteration({'iteration': i, 'loss': F_new.item(), 'func evals': closure_eval, 'LS success': not fail,
                                    'dist val': self.corr_obj.Dc.item(), 'reg val': self.corr_obj.Sc.item(), 'grad norm': torch.norm(g_new).item(),
                                    'step size': float(t)})
            fevals = fevals + closure_eval
            if fail:
                self.log.log_message(f"line search failed: took {ls_step} steps without finding good stepsize")
                break
            else:
                stop_grad = torch.norm(g_new) <= tolG
                stop_func_change = i > 1 and (torch.abs(F_new - self.log.history[-2]['loss'])) <= tolJ * (1 + Jstop)
                stop_grad_relative = torch.norm(g_new) <= tolG * (1 + Jstop)
                ...
                if stop_fmap_change:
                    ...
                    break
            Bc_old = torch.clone(self.Bc).detach()
    ...
    def run_correction_use_autograd(self, B0):
        """
        Run optimization (using autograd).
        ...
        tolG = 1e-2  # for stopping, gradient norm
        tolJ = 1e-3
        tolY = 1e-2

        lbfgs = FullBatchLBFGS([self.Bc])

        def closure():
            lbfgs.zero_grad()
            objective = self.corr_obj.eval(self.Bc, do_derivative=False)
            objective.backward()
            return objective

        loss_init = self.corr_obj.eval(self.B0, do_derivative=False).detach().item()
        self.log.log_iteration(
            {'iteration': "Iteration", 'loss': "Loss Value", 'func evals': "Num Func Evals", 'LS success': "LS Success",
             'dist val': "Dist Val", 'reg val': "Reg Val", 'grad norm': "Grad Norm",
             'step size': "Step Size"})
        self.log.log_iteration({'iteration': 0, 'loss': loss_init})
        Jstop = abs(loss_init)
        ...
        fevals = 0
        for i in range(1, self.max_iter + 1):
            Bc_old = self.Bc.clone()
            options = {'closure': closure, 'current_loss': torch.tensor(loss_init, dtype=self.Bc.dtype, device=self.Bc.device), 'max_ls': 25}
            F_new, g_new, t, ls_step, closure_eval, grad_eval, desc_dir, fail = lbfgs.step(options)
            stop_fmap_change = i > 1 and (torch.norm(self.Bc - Bc_old)) <= tolY * (1 + torch.norm(B0))
            self.log.log_iteration(
                {'iteration': i, 'loss': F_new.item(), 'func evals': closure_eval, 'LS success': not fail,
                 'dist val': self.corr_obj.Dc.item(), 'reg val': self.corr_obj.Sc.item(),
                 'grad norm': torch.norm(g_new).item(),
                 'step size': float(t)})
            fevals = fevals + closure_eval
            if fail:
                self.log.log_message(f"line search failed: took {ls_step} steps without finding good stepsize")
...
class LBFGSSolve(Optimizer):
    ...
    def __init__(self, params, lr=1., history_size=10, line_search='Wolfe',
        ...
        super(LBFGSSolve, self).__init__(params, defaults)
...
class FullBatchLBFGS(LBFGSSolve):
    """
